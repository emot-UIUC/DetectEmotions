...
/**
 * A class which instantiates and runs a FrameDetector on a background thread. The background thread
 * is defined in an inner class and is re-created for any pair of start() stop() calls the user
 * makes.
 */
public class AsyncFrameDetector {
    ...
    /**
     * Starts running FrameDetector on a background thread. Note that FrameDetector is not
     * guaranteed to have started by the time this call returns, because it is started
     * asynchronously.
     */
    public void start() {
        if (isRunning)
    ...
    /**
     * Since our handler may be disposed long before our AsyncFrameDetector is disposed, we declare
     * it as a static inner class and hold only a WeakReference to our AsyncFrameDetector object, to
     * avoid memory leaks.
     */
    ...
    /**
     * Our background thread, which operates by instantiating a FrameDetector in the background and
     * communicating with it via Handler Messages. See Android HandlerThread class documentation for
    ...
    private class FrameDetectorThread extends HandlerThread {
        ...
        /**
         * The getLooper() method blocks the calling thread, so it will cause our calling thread to
         * block until this thread is ready to start receiving messages. This should ensure that the
         * threadHandler object is not null in any of the public methods below.
         */
        ...
        /**
         * When resetting or stopping the detector, we don't want our command to have to wait for
         * messages in front of it to finish processing, so we purge any non-critical messages,
         * namely PROCESS_FRAME and RESET_DETECTOR.
         */
