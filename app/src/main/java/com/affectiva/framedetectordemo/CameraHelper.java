...
/**
 * The CameraHelper class controls the Camera by responding to the lifecycle events of
 * SurfaceHolder. It is important not to miss the SurfaceCreated event fired by SurfaceHolder; thus,
 * CameraHelper should be instantiated at the start of its host Activity (which should happen as
 * long as the developer instantiates CameraDetector at the start of its host Activity) (e.g. in
 * onCreate()).
 */
class CameraHelper extends OrientationEventListener implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    CameraHelper(Context context, SurfaceView providedSurfaceView, Display defaultDisplay) {
        super(context);
    ...
    /**
     * Stops the camera. If a camera was in the process of being created, this method will attempt
     * to block until the camera has been released.
     */
    ...
    /**
     * A wrapper class to encapsulate the camera and its properties. This is meant to contain
     * instance-specific data. This class is also used as a synchronization lock and thus serves to
     * ensure both the background and foreground thread have access to the latest data regarding the
